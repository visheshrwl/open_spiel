...
class SFLPTest(parameterized.TestCase):
  ...
  def test_sequential_equilibrium_runs(self, name):
  ...
  def test_sequential_equilibrium_guess_the_ace(self):
  ...
  def test_sequential_equilibrium_kuhn_poker(self):
    ...
    filename = file_utils.find_file(
        "open_spiel/games/efg_game/games/kuhn_poker_with_raise.efg",
        2,
    )
    game = pyspiel.load_game("efg_game(filename=" + filename + ")")
    ...
    # Player 1 holds the 2 (King), (B)ets, and suddenly faces a (R)aise from
    # Player 2.
    # A rational Player 2 will only raise if they hold the King.
    # Because Player 1 is holding the King, it is impossible for Player 2 to
    # have it.
